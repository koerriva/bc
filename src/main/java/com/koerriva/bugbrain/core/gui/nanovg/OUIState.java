package com.koerriva.bugbrain.core.gui.nanovg;
...
public class OUIState {
    ...
    private static int colorrect(ByteBuffer label, NVGColor color) {
        ...
        data.head().subtype(SubType.ST_RECT.ordinal());
    ...
    private static int label(int iconid, ByteBuffer label) {
        ...
        data.head().subtype(SubType.ST_LABEL.ordinal());
        data.head().handler(null);
        data.iconid(iconid);
        data.label(label);
        return item;
    ...
    private static int button(int iconid, ByteBuffer label, UIHandler handler) {
        ...
        UIButtonData data = UIButtonData.create(nuiAllocHandle(item, UIButtonData.SIZEOF));
        data.head().subtype(SubType.ST_BUTTON.ordinal());
    ...
    private static int check(ByteBuffer label, IntBuffer option) {
        ...
        data.head().subtype(SubType.ST_CHECK.ordinal());
    ...
    private int slider(ByteBuffer label, FloatBuffer progress) {
        ...
        data.head().subtype(SubType.ST_SLIDER.ordinal());
    ...
    private int textbox(ByteBuffer text, int maxsize) {
        ...
        data.head().subtype(SubType.ST_TEXT.ordinal());
    ...
    private int radio(int iconid, @Nullable ByteBuffer label, IntBuffer value) {
        ...
        data.head().subtype(SubType.ST_RADIO.ordinal());
    ...
    private static int panel() {
        ...
        data.subtype(SubType.ST_PANEL.ordinal());
        data.handler(null);
        return item;
    }
    ...
    private static int hbox() {
        int    item = uiItem();
        UIData data = UIData.create(nuiAllocHandle(item, UIData.SIZEOF));
        data.subtype(SubType.ST_HBOX.ordinal());
        data.handler(null);
    ...
    private static int vbox() {
        int    item = uiItem();
        UIData data = UIData.create(nuiAllocHandle(item, UIData.SIZEOF));
        data.subtype(SubType.ST_VBOX.ordinal());
        data.handler(null);
    ...
    public void draw(long vg, float w, float h) {
        ...
        if (choice.get(0) == opt_blendish_demo) {
            ...
            data.handler(null);
            data.subtype(SubType.ST_DEMOSTUFF.ordinal());
