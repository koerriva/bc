...
import com.koerriva.bugbrain.core.gui.nuklear.Editor;
...
public class NuklearGUI implements GUI{
    ...
    private int width,height,display_width,display_height;

    private final Editor ui = new Editor();

    public NuklearGUI() {
        try {
            this.ttf = ioResourceToByteBuffer("data/font/NotoSansSC-Bold.otf", 9 * 1024 * 1024);
        } catch (IOException e) {
    ...
    public void init(Window window) {
        ctx = setupWindow(window.getHandle());
        setupFont();
    }
    ...
    public void render(Window window) {
        newFrame(window.getHandle());
        width = window.size.width;
        height = window.size.height;
        display_width = window.size.frameBufferWidth;
        display_height = window.size.frameBufferHeight;
...
//        STBTTPackedchar.Buffer cdata    = STBTTPackedchar.create(95);
        STBTTPackedchar.Buffer cdata    = STBTTPackedchar.create(0xFFF1);
